...
Click on `File` > `Set game location folder...` and select folder where game is installed.
...
The files MUS.TAB, MUS.DAT, MUSFRA.ANI, MUSELE.ANI, INPAL.DAT (floppy version only) are RNC compressed and should be decompressed with a specific tool (otherwise they will be ignored). You can use tool provided on this [page](https://github.com/lab313ru/rnc_propack_source/releases).

Put the following lines in a batch file and execute it:
```batch
