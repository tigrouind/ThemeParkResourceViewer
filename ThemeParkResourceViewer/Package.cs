...
namespace ThemeParkResourceViewer
{
	static class Package
	{
		...
		static readonly Dictionary<uint, byte[]> pixelsCache = new Dictionary<uint, byte[]>();
		...
		public static void LoadAnim(string gameFolder, string filename)
		{
			switch (filename)
			{
				case "MSTA-0":
					...
					if (filename != lastAnim)
					{
						...
						Animations = FirstFrame.Select(GetAnimation).ToArray();
						lastAnim = filename;
					}
					break;
			...
			Animation GetAnimation(ushort frameIndex)
			{
