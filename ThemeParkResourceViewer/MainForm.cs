	...
	public partial class MainForm : Form
	{
		...
		void LoadTreeView()
		{
				...
				.OrderBy(x => x))
			{
				...
				{
					GetNode("FLI", "MOVIES").Nodes.Add(fileName, $"{fileName} ({MoviePlayer.GetDuration(Path.Combine(gameFolder, fileName)):m\\:ss})");
				}

				if ((fileName == "TAKOVER.ANM" || fileName == "BUSTED.ANM")
					&& allfiles.Contains(fileNameWithoutExtension + ".DAT"))
				{
					GetNode("FLI", "MOVIES").Nodes.Add(fileName, $"{fileName} ({MoviePlayer.GetDuration(Path.Combine(gameFolder, fileName), 0):m\\:ss})");
				}
				...
				if (fileName.StartsWith("SND") && Path.GetExtension(fileName) == ".TAB")
				{
					...
					node = GetNode("SND" + lastChar, new[] { "11KHZ 8-BIT", "22KHZ 8-BIT", "44KHZ 16-BIT" }[lastChar], node.Nodes).Nodes.Add(fileNameWithoutExtension, fileNameWithoutExtension);
					node.Nodes.Add("");
				}
				...
				if (fileName.StartsWith("MUSIC") && Path.GetExtension(fileName) == ".TAB")
				{
					...
					node = GetNode("MUS" + lastChar, new[] { "ADLIB", "ROLAND", "GENERAL MIDI" }[lastChar], node.Nodes).Nodes.Add(fileNameWithoutExtension, fileNameWithoutExtension);
					node.Nodes.Add("");
				}
			}
		}
				...
				case "FLI|1":
					{
						...
						trackBar1.Value = 0;
						trackBar1.Maximum = moviePlayer.FrameCount;
						panel1.Visible = true;
					}
					break;
				...
				case "MUS|3":
					{
						...
						panel1.Visible = true;
						trackBar1.Value = 0;
						trackBar1.Maximum = (int)MusicPlayer.TotalTime.TotalMilliseconds;
					...
					case "FLI|1":
						{
							...
							{
								trackBar1.Maximum = moviePlayer.CurrentFrame; //FrameCount might be more than reality
							...
							if (!ignoreTrackbar)
							{
								labTime.Text = $"{TimeSpan.FromMilliseconds(moviePlayer.CurrentFrame * 1000 / 15):mm\\:ss}";
								trackBar1.Value = Math.Min(moviePlayer.CurrentFrame, moviePlayer.FrameCount);
							}
						}
						break;
					...
					case "MUS|3":
						{
							if (!ignoreTrackbar)
							{
								labTime.Text = $"{MusicPlayer.CurrentTime:mm\\:ss}";
								trackBar1.Value = Math.Min((int)MusicPlayer.CurrentTime.TotalMilliseconds, trackBar1.Maximum);
							}
						}
						break;
				}
			...
			if (e.Button == MouseButtons.Left)
			{
				...
				if (node != null)
				{
					...
					switch ($"{root.Name}|{node.Level}")
					{
						...
						case "MUS|3":
							labTime.Text = $"{TimeSpan.FromMilliseconds(trackBar1.Value):mm\\:ss}";
							MusicPlayer.Seek(TimeSpan.FromMilliseconds(trackBar1.Value));
							timer.Start();
							break;
					}
				}
		...
		void SetTrackPosition(int mouse)
		{
			const int border = 14;
			double ratio = (double)(mouse - border) / (trackBar1.Width - border * 2);
			int newValue = (int)Math.Round(ratio * trackBar1.Maximum);
