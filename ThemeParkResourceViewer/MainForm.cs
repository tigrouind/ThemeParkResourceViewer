					...
					case "ANI|2":
						{
							...
							{
								FileName = $"{node.Parent.Text}_{node.Name}.png",
								Filter = "PNG individual frames (*.png)|*.png"
							};
							...
							if (dialog.ShowDialog() == DialogResult.OK)
							{
								...
								foreach (SpriteFrame frame in Package.GetAllFrames(Package.SpriteFrames[firstFrame.Next]))
								{
									if (frame.Width > 0 && frame.Height > 0)
									{
										...
										pictureBox.Image.Save(Path.Combine(Path.GetDirectoryName(dialog.FileName),
											$"{Path.GetFileNameWithoutExtension(dialog.FileName)}_{index:0000}.png"), ImageFormat.Png);
									}
									index++;
								}
							}
					...
					case "FLI|1":
						{
							...
							if (dialog.ShowDialog() == DialogResult.OK)
							{
								...
								{
									bitmap.Bitmap.Save(Path.Combine(Path.GetDirectoryName(dialog.FileName),
										$"{Path.GetFileNameWithoutExtension(dialog.FileName)}_{index:0000}.png"), ImageFormat.Png);
									index++;
								}
							}
