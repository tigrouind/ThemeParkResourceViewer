...
namespace ThemeParkResourceViewer
{
	public partial class MainForm : Form
	{
		...
		byte[] soundBytes;
		readonly Stack<TreeNode> undo = new Stack<TreeNode>();
		readonly Stack<TreeNode> redo = new Stack<TreeNode>();
		...
		bool TryGetDataFolder(string folder, out string result)
		{
		...
		public TreeNode GetNode(string key, string text, TreeNodeCollection nodes)
		{
		...
		void LoadTreeView()
		{
				...
				.OrderBy(x => x))
			{
				...
				if ((fileNameWithoutExtension == "MSTA-0" && anim.All(x => allfiles.Contains(x)))
					|| (fileNameWithoutExtension == "MUSSTA" && animUS.All(x => allfiles.Contains(x))))
				{
			...
			void AnimateSprite(Animation animation)
			{
				...
				if (currentFrame.Width > 0 && currentFrame.Height > 0)
				{
					...
					if (showAnimationElementsToolStripMenuItem.Checked)
					{
						...
						{
							SpriteTab tab = Package.SpriteTabs[element.Sprite];
							SpriteViewer.DrawSpriteRectangle(bitmap, tab,
								animation.Left + element.XOffset,
								animation.Top + element.YOffset);
						}
					}
				...
				foreach (var anim in Package.Animations)
				{
					if (anim.Width > 0 && anim.Height > 0)
					{
						...
						if (anim.LastFrame != anim.CurrentFrame &&
							(currentFrame.FirstElement != Package.SpriteFrames[anim.LastFrame].FirstElement)
							|| clearAnimation)
						{
							int posX = index % tilesPerRow * SpriteViewer.TileSize;
							int posY = index / tilesPerRow * SpriteViewer.TileSize;
							SpriteViewer.ClearSprite(bitmap, posX, posY);
							...
							if (currentFrame.Width > 0 && currentFrame.Height > 0)
							{
								...
								void DrawAnimedSprite()
								{
									...
									if (showAnimationElementsToolStripMenuItem.Checked && anim.Width <= SpriteViewer.TileSize && anim.Height <= SpriteViewer.TileSize)
									{
										foreach (SpriteElement element in Package.GetAllElements(Package.SpriteElements[currentFrame.FirstElement]))
										{
											SpriteTab tab = Package.SpriteTabs[element.Sprite];
											SpriteViewer.DrawSpriteRectangle(bitmap, tab,
												posX + anim.Left + element.XOffset,
												posY + anim.Top + element.YOffset);
										}
									}
								}
							}
		...
		async void ExportToPNGToolStripMenuItem_Click(object sender, EventArgs e)
		{
					...
					case "MUS|3":
						{
							SaveFileDialog dialog = new SaveFileDialog
							{
								FileName = $"{node.Text.Split('.')[0]}.hmp",
								Filter = $"HMP (*.hmp)|*.hmp"
							};
					...
					case "FLI|1":
						{
							...
							if (dialog.ShowDialog() == DialogResult.OK)
							{
								...
								IProgress<string> progress = new Progress<string>(x => Text = x.ToString());

								await Task.Run(() =>
								{
									int index = 1;
									moviePlayer.Reset();
									var filePath = Path.Combine(Path.GetDirectoryName(dialog.FileName), $"{Path.GetFileNameWithoutExtension(dialog.FileName)}.gif");
									using (var stream = new FileStream(filePath, FileMode.Create, FileAccess.Write, FileShare.Write, 1024 * 1024))
									using (var gif = new AnimatedGifCreator(stream, 66))
									{
										while (moviePlayer.RenderFrame(bitmap))
										{
											gif.AddFrame(bitmap.Bitmap, delay: -1, quality: GifQuality.Bit8);
											progress.Report($"{index++}/{moviePlayer.FrameCount}");
										}
									}
								});
		...
		{
			pictureBox.InterpolationMode = pictureBox.InterpolationMode == InterpolationMode.NearestNeighbor ?
		...
		void UndoRedo(MouseEventArgs e)
		{
			switch (e.Button)
			{
				case MouseButtons.XButton1: //back
					if (undo.Count > 0)
					{
						redo.Push(treeView.SelectedNode);
						ignoreSelectedNodeEvent = true;
						treeView.SelectedNode = undo.Pop();
				...
				case MouseButtons.XButton2: //forward
					if (redo.Count > 0)
					{
						undo.Push(treeView.SelectedNode);
						ignoreSelectedNodeEvent = true;
						treeView.SelectedNode = redo.Pop();
						ignoreSelectedNodeEvent = false;
					}
					break;
			}
