	...
	public partial class MainForm : Form
	{
		...
		void LoadTreeView()
		{
				...
				.OrderBy(x => x))
			{
				...
				{
					TreeNode node = GetNode("ANI", "ANIMATIONS", 1).Nodes.Add(fileNameWithoutExtension, fileNameWithoutExtension).SetStateImageIndex(2);
					node.Nodes.Add("");
				}
			...
			if (node != null)
			{
				...
				switch ($"{root.Name}|{node.Level}")
				{
					...
					case "MUS|3":
						{
							UpdateTrackBarText();
							trackBar1.Value = Math.Min((int)MusicPlayer.CurrentTime.TotalMilliseconds, (int)MusicPlayer.TotalTime.TotalMilliseconds);
						}
						break;
				}
