...
namespace ThemeParkResourceViewer
{
	public partial class MainForm : Form
	{
		...
		byte[] soundBytes;
		readonly Stack<(TreeNode, int)> undo = new Stack<(TreeNode, int)>();
		readonly Stack<(TreeNode, int)> redo = new Stack<(TreeNode, int)>();
					...
					case "FLI|1":
						{
							SaveFileDialog dialog = new SaveFileDialog
							{
								FileName = $"{node.Text}.avi",
								Filter = "AVI (*.avi)|*.avi"
							};

							if (dialog.ShowDialog() == DialogResult.OK)
							{
								var filePath = Path.Combine(Path.GetDirectoryName(dialog.FileName), $"{Path.GetFileNameWithoutExtension(dialog.FileName)}.avi");

								FFmpegLoader.FFmpegPath = @"FFmpeg";
								var videoParams = new VideoEncoderSettings(bitmap.Width, bitmap.Height, framerate: 15, VideoCodec.H264)
								{
									CRF = 0,
									EncoderPreset = EncoderPreset.VerySlow,
								};

								using var file = MediaBuilder.CreateContainer(filePath).WithVideo(videoParams).Create();

								moviePlayer.Reset();
								while (moviePlayer.RenderFrame(bitmap))
								{
									byte[] byteArray = new byte[bitmap.Bits.Length * sizeof(uint)];
									Buffer.BlockCopy(bitmap.Bits, 0, byteArray, 0, byteArray.Length);

									var imageData = ImageData.FromArray(byteArray, ImagePixelFormat.Bgra32, new System.Drawing.Size(bitmap.Width, bitmap.Height));
