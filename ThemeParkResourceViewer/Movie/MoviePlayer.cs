...
namespace ThemeParkResourceViewer
{
	public class MoviePlayer : IDisposable
	{
		...
		public MoviePlayer(string filename, int header = 12)
		{
			reader = new BinaryReader(File.OpenRead(filename));
			fileLength = reader.BaseStream.Length;

			reader.BaseStream.Seek(header, SeekOrigin.Begin); //header size (4) + 0xAF12 magic + frame count (2) + width (2) + height (2)
		...
		public bool RenderFrame(DirectBitmap bitmap)
		{
